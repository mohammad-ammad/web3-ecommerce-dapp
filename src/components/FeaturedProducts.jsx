  ...
  return (
    <div className="py-10 px-2 md:px-24 bg-slate-100">
        ...
        <div className='flex justify-start items-center my-2 py-1 bg-white rounded-full w-44 mx-auto md:float-right shadow-inner shadow-slate-300 cursor-pointer'>
            <div className={`mx-1 text-sm ${toggle === false ? 'bg-black text-white px-3 py-1 rounded-full' : ''} cursor-pointer`} onClick={()=>setToggle(false)}>US dollar</div>
            <div className={`mx-1 text-sm ${toggle === true ? 'bg-black text-white  px-6 py-1 rounded-full' : ''} cursor-pointer`} onClick={()=>setToggle(true)}>Crypto</div>
        </div>
